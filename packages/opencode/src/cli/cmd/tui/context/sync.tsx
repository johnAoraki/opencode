            ...
            setStore(
              produce((draft) => {
                ...
                for (const message of fetched) {
                  ...
                  const parts = (merged.parts.get(message.info.id) ?? message.parts).flatMap((part) => {
                ...
                for (const message of visible) {
                  if (fetched.some((item) => item.info.id === message.id)) continue
                  const currentParts = draft.part[message.id] ?? []
                  const parts = merged.parts.get(message.id) ?? currentParts
